...
各板子进入刷机模式的方式**并不相同**（RP2040/RP2350 是按住 BOOTSEL 插入，
ESP32 是 BOOT+RESET 的组合），所以 App 里不再写死任何手势——它只提示"让板子进入
刷机模式"，具体做法请参照你板子自己的说明。进下载模式后设备名通常会变
（ESP32 会变成 "USB JTAG/serial debug unit"）。
