...
**本仓库的代码由 AI 编写，并非 PicoKey 上游作者的作品。** 设备出现异常时，
...
协议层有自动化自检覆盖，但**全部代码从未在真实 PicoKey 上运行过**：
...
| --- | --- | --- | --- |
| **RP2040** | Raspberry Pi Pico / Pico W | UF2（BOOTSEL 变 U 盘） | ❌ 无硬件保护 |
| **RP2350** | Pico 2、Waveshare RP2350-One/Zero/Tiny | UF2（BOOTSEL 变 U 盘） | ✅ 完整支持 |
...
那不是 PicoKey 的 CCID 接口，对它发指令不会有任何回应。日志里看到这个名字，说明板子
卡在下载模式——**拔掉、什么都不按、重新插**。
...
**一键切换固件没有实现**：官方桌面版把各固件镜像打包在应用里，本项目既没有这些镜像，
也没有再分发的权利。要换固件请进刷机模式后自行写入。

---

## 使用方法

1. 用 **OTG 转接线**连接 PicoKey。**不要按任何按键**——按住 BOOT/BOOTSEL 插入会让板子
   进刷机模式，此时它不是 PicoKey，App 连不上。
...
| 板子 | 底层机制 | 本 App 的做法 |
|---|---|---|
| RP2040 / RP2350 | BOOTSEL 后 flash 变成 U 盘，拷入 `.uf2` 即完成 | 把 UF2 交给系统文件管理器，由你存到 `RPI-RP2` / `RP2350` 盘里 |
| ESP32-S2 / S3 | USB Serial/JTAG 上的 esptool ROM 协议，没有 UF2 bootloader | 直接实现该协议，不需要外部工具 |

固件可来自本地文件或 https 地址，加载后 App 会识别格式（UF2 / ESP 镜像 / ZIP / gzip /
误下载成网页）并显示大小和适用芯片。
