...
# Android port: never sys.exit() at import time (that would kill the APK on
# startup). The modules are optional and only needed for DKEK / secure channel
# operations; a clear exception is raised the moment they are really used.
try:
...
def _require(name, obj):
    if obj is None:
        raise ImportError(
            f'{name} is not available in this build. Add it to buildozer.spec '
            f'requirements (pycvc / cryptography) if you need secure channel support.')
    return obj
